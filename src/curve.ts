...
export function areaToGeometry(coordinates: [number, number][]) {
  ...
  const curve = new CatmullRomCurve3(outlinePoints, true);
  const curveLength = curve.getLength();
  const curveSegments = Math.round(curveLength * 200);
  const curvePoints = curve.getPoints(curveSegments);

  const curveCoordinates = curvePoints.map(point => vec3ToCoordinate(point)).map(coord => [coord[1], coord[0]]);

  const poly = polygon([curveCoordinates]);
  const box = bbox(poly);
  const [minX, minY, maxX, maxY] = box;
  const step = 1;

  const points = curveCoordinates.slice();

  for (let latitude = minY; latitude <= maxY; latitude += step) {
    for (let longitude = minX; longitude <= maxX; longitude += step) {
      if (booleanPointInPolygon(point([longitude, latitude]), poly)) {
  ...
  // ctx.lineWidth = 1;
  // const scaleToCanvas = (point: number[]) => {
  //   const [x, y] = point;
  //   const [minX, minY, maxX, maxY] = box;
  ...
  for (const i of indices) {
    const vertex = coordinateToVec3(points[i][1], points[i][0]);
    vertices.push(vertex.x, vertex.y, vertex.z);
    total.add(vertex);
  }
  ...
  const averagePoint = total.clone().divideScalar(indices.length);
